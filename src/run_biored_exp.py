...
class DatasetProcessor(object):
    ...
    def _get_dataset(self, 
        ...
        if has_header:
            data_df = pd.read_csv(data_file, sep='\t', dtype=str, keep_default_na=False)
        else:
            data_df = pd.read_csv(data_file, sep='\t', header=None, dtype=str, keep_default_na=False)
        data_dict = {}
...
class BioREDMultiProcessor(DatasetProcessor):
...
class BioREDNoveltyProcessor(DatasetProcessor):
...
def main():
    ...
    processors = {
        "biored_all_mul": BioREDMultiProcessor,
        "biored_novelty": BioREDNoveltyProcessor,
    }
    ...
    def compute_metrics(p: EvalPrediction) -> Dict:
        ...
        np_f_tp       = np.float64(np_array_tp_wo_neg.shape[0])
        np_f_pred_pos = np.float64(np_array_pred_pos.shape[0])
        np_f_gold_pos = np.float64(np_array_gold_pos.shape[0])
