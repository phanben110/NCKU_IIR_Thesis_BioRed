...
parser.add_option('--in_pubtator_file',       action="store",
                     dest="in_pubtator_file", help="input pubtator file",
                                              default="")

parser.add_option('--out_tsv_file',           action="store",
                     dest="out_tsv_file",     help="output processed tsv file for BERT",
                                              default="")

parser.add_option('--in_data_dir',            action="store",
                     dest="in_data_dir",      help="input data folder containing gold pubtator file",
                                              default="datasets/biored/BioRED/")

parser.add_option('--out_data_dir',           action="store",
                     dest="out_data_dir",     help="output data folder containing processed tsv files for BERT",
                                              default="datasets/biored/processed/")
        ...
        for line in pub_reader:
            ...
            if len(tks) > 1 and (tks[1] == 't' or tks[1] == 'a'):
                ...
                if len(_tks) == 6:
                    ...
                    ids = re.split(re_id_spliter_str, _tks[5])
...
def convert_pubtator_to_tsv_file(
    ...
    if out_data_dir != '' and (not os.path.exists(out_data_dir)):
...
if __name__ == '__main__':
    ...
    if options.in_pubtator_file != '' and options.out_tsv_file != '':
        convert_pubtator_to_tsv_file(
            ...
            src_tgt_pairs        = src_tgt_pairs,
            has_novelty          = True)
    elif options.in_data_dir != '' and options.out_data_dir != '':
        gen_biored_dataset(
